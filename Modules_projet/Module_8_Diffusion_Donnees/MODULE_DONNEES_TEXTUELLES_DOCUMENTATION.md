...
### Qu'est-ce que l'algo Hécate ?

**Hécate** est un script d'export automatisé vers Nakala.

**Fonctionnalités** :
- Préparation des packages conformes Nakala
- Génération métadonnées
- Upload automatique via API Nakala
- Gestion des erreurs
- Logs détaillés
...
- Python
- API REST Nakala
- Gestion des uploads volumineux
- Retry en cas d'échec réseau

**Usage** :
```bash
python algo_hecate.py \
  --input /path/to/corpus/ \
  --metadata metadata.json \
  --nakala-api-key YOUR_KEY
```
...
### Répartition estimée (d'après Module 3)

Sur **317 manuscrits** du corpus initial :

**Libre de droit** : ~190 manuscrits (60%)
- Candidats à publication Nakala
- Visibilité maximale

**Pas libre** : ~127 manuscrits (40%)
- Stockage Seafile privé
- Usage recherche interne

### Impact sur la diffusion

**Publications Nakala attendues** : ~190 corpus
- Accessibles mondialement
- Cités dans publications académiques
- DOI attribués
- Impact recherche maximal

**Corpus privés Seafile** : ~127 corpus
- Accès équipe uniquement
- Utilisation interne pour recherche
- Possibilité publication ultérieure (si droits évoluent)
...
**Sources des données textuelles** :

1. **Module 6 (PAGEtopage)** → Corpus génériques enrichis
   - Textes lemmatisés
   - Formats clean/diplomatic/annotated
   - Métadonnées incluses

2. **Pipeline Décret de Gratien** → Corpus spécifique Décret
   - Format .txt adapté
   - Structure allégations + canons
   - Enrichissement spécifique

**Statut des droits** : Défini dès le Module 3 (Récupération d'éditions)

### En parallèle

**Module Fiches Métadonnées** : Les métadonnées Heurist sont utilisées pour enrichir les packages Nakala.
